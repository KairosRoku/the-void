...
public class RaycastShoot : MonoBehaviour
{
    ...
    private Dictionary<Transform, int> hitCounts = new Dictionary<Transform, int>(); // Dictionary to track hit counts for enemies
                ...
                if (Physics.Raycast(currentPosition, shootDirection, out hit, range))
                {
                        ...
                        hit.collider.CompareTag("return"))
                    {
                        ...
                        if (hit.collider.CompareTag("enemy"))
                        {
                            ...
                            // Increment the hit count for the enemy
                            if (!hitCounts.ContainsKey(hit.collider.transform))
                            {
                                hitCounts[hit.collider.transform] = 0;
                            }
                            hitCounts[hit.collider.transform]++;

                            // Check if the enemy has reached the required number of hits to play the audio cue
                            if (hitCounts[hit.collider.transform] >= hitsRequiredForAudio)
                            {
                                PlayRandomAudioCue();
                                hitCounts[hit.collider.transform] = 0; // Reset the hit count after playing the audio cue
                            }

                            // Update the last tagged time for the enemy
